...
fn animate_light_beings(
    ...
    mut beings: Query<(&mut Transform, &LightBeing, &MeshMaterial3d<StandardMaterial>)>,
    mut mats: ResMut<Assets<StandardMaterial>>,
) {
    let t = time.elapsed_secs();
    
    for (mut transform, being, mat_handle) in beings.iter_mut() {
        if being.room != player.room { continue; }
...
fn animate_halos(
    ...
    for (mut transform, halo, mat_handle) in halos.iter_mut() {
        if halo.room != player.room { continue; }
