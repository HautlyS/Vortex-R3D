...
impl Default for EnvironmentMood {
    fn default() -> Self {
        Self { warmth: 0.5, light_intensity: 1.0, is_night: false }
    }
}
...
fn detect_environment_mood(
    ibl: Option<Res<IblLightProbe>>,
    mut mood: ResMut<EnvironmentMood>,
) {
    let Some(ibl) = ibl else { return };
    if !ibl.analyzed || !ibl.is_changed() { return };
    ...
    if mood.is_night {
        ...
        info!("☀️ Daylight detected (warmth={:.2}, intensity={:.2})", mood.warmth, mood.light_intensity);
    }
}
