...
use super::{IblLightProbe, AnalyzePanoramaEvent, IblDirectionalLight, IblReadyEvent, SphericalHarmonics};
...
pub fn analyze_panorama_system(
    ...
    for _ in events.read() {
        if light_probe.analyzed { continue; }

        let Some(image) = images.get(&panorama_assets.demo_panorama) else { continue };
...
pub fn receive_analysis_results(
    ...
    if let Ok(result) = channel.rx.try_recv() {
        ...
        info!("✅ IBL: dir={:?}, intensity={:.0}, exposure={:.2}", 
...
fn analyze_image(data: &[u8], width: usize, height: usize, bpp: usize) -> IblAnalysisResult {
    ...
    let intensity = if contrast > 3.0 { 50000.0 + brightest * 50000.0 }
        else if contrast > 1.5 { 10000.0 + brightest * 40000.0 }
        else { 1000.0 + brightest * 9000.0 };
    ...
    IblAnalysisResult {
        ...
        ambient_color: Color::srgb(ambient.x.clamp(0.0, 1.0), ambient.y.clamp(0.0, 1.0), ambient.z.clamp(0.0, 1.0)),
...
pub fn apply_ibl_lighting_system(
    ...
    if !light_probe.analyzed || !light_probe.is_changed() { return; }
